...
public final class CNCMachineButton extends DeviceButton implements CNCMachineListener {
    ...
    private Label[] lblExtraInfoList;
    ...
    private static final String CSS_CLASS_DEVICE_ACTIVE = "device-info-label-active";

    public CNCMachineButton(DeviceInformation deviceInfo) {
        super(deviceInfo);
        addExtraInfoLabels();
        addLabelToBox(lblExtraInfoList);
        ...
        Platform.runLater(new Thread() {
            ...
            public void run() {
                if ((machine.isConnected()) && ((machine.getWayOfOperating() == EWayOfOperating.M_CODES) || (machine.getWayOfOperating() == EWayOfOperating.M_CODES_DUAL_LOAD))) {
                    lblExtraInfoList[0].getStyleClass().remove(CSS_CLASS_DEVICE_ACTIVE);
                    lblExtraInfoList[2].getStyleClass().remove(CSS_CLASS_DEVICE_ACTIVE);
                    if (machine.getMCodeAdapter().getActiveMCodes().size() > 0) {
                        if (machine.getMCodeAdapter().getActiveMCodes().contains(mcodesForDeviceStep[0])) {
                            lblExtraInfoList[0].getStyleClass().add(CSS_CLASS_DEVICE_ACTIVE);
                        }
                        if (machine.getMCodeAdapter().getActiveMCodes().contains(mcodesForDeviceStep[1])) {
                            lblExtraInfoList[2].getStyleClass().add(CSS_CLASS_DEVICE_ACTIVE);
                        } 
                    } 
                }
            }
    ...
    private void addExtraInfoLabels() {
        lblExtraInfoList = new Label[3];
        if (mcodesForDeviceStep[0] != null) {
            String txt = "M" + (mcodesForDeviceStep[0] + 1);
            lblExtraInfoList[0] = makeExtraInfoLabel(txt);
        } 
        if (machine.hasWorkNumberSearch()) {
            String txt = "P" + getDeviceInformation().getProcessingStep().getDeviceSettings().getWorkNumber();
            lblExtraInfoList[1] = makeExtraInfoLabel(txt);
        }
        if (mcodesForDeviceStep[1] != null) {
            String txt = "M" + (mcodesForDeviceStep[1] + 1);
            lblExtraInfoList[2] = makeExtraInfoLabel(txt);
        }
    }
    
    private Label makeExtraInfoLabel(String text) {
        Label lblExtraInfo = new Label(text);
...
//        for (Label lbl: lblExtraInfoList) {
//            if (lbl != null) {
//                lbl.getStyleClass().remove(CSS_CLASS_UNCLICKABLE_INFO);
//                if (!clickable) {
//                    lbl.getStyleClass().add(CSS_CLASS_UNCLICKABLE_INFO);
//                }
//            }
//        }
    }
