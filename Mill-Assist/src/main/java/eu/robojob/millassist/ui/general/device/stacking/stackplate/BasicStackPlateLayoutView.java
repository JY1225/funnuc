            ...
            public void run() {
                ...
                if (basicStackPlate.getLayout().getStackingPositions().size() > 0) {
                    if(basicStackPlate.hasGridPlate()) {
                        ...
                        layoutPane.getChildren().add(getGridPlateView(layout.getGridPlate()));
                    } else {
                        configureStuds();
                    }
                    configureWorkPieces();
    ...
    private synchronized void configureWorkPieces() {
        ...
        for (StackPlateStackingPosition stackingPosition : stackingPositions) {
            if (stackingPosition.getWorkPiece() != null) {
                ...
                markerPane.setLayoutX(stackingPosition.getPosition().getX() - workPieceRepre.getXCorrection());
                //LayoutY - the origin of the piece (left bottom corner)
                markerPane.setLayoutY(width - stackingPosition.getPosition().getY() - workPieceRepre.getYCorrection());
                if (basicStackPlate.getBasicLayout().isRightAligned() && stackingPosition.getOrientation() == 45) {
                    markerPane.setRotate(stackingPosition.getOrientation());
                } else {
                    markerPane.setRotate(stackingPosition.getOrientation()*-1);
                }
