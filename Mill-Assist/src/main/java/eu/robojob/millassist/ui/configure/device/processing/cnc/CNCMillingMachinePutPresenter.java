...
public class CNCMillingMachinePutPresenter extends AbstractFormPresenter<CNCMillingMachinePutView, CNCMillingMachineMenuPresenter> {
	...
	public void resetAirblow(String clampingName) {
		Clamping clamping = putStep.getRobotSettings().getWorkArea().getWorkAreaManager().getClampingByName(clampingName);
		if (clamping != null) {
	...
	public void changedAirblow(final boolean airblow) {
		if (putStep.getRobotSettings().isRobotAirblow() != airblow) {
			putStep.getRobotSettings().setRobotAirblow(airblow);
			if (!airblow) {
	...
	void changedClamping(String clampingName) {
		if (clampingName != null) {
			int clampingId = putStep.getRobotSettings().getWorkArea().getWorkAreaManager().getClampingByName(clampingName).getId();
	...
	public boolean isConfigured() {
		if (putStep.getRobotSettings().isRobotAirblow() && putStep.getRobotSettings().getRobotAirblowSettings().isEmpty()) {
			getView().showNotification(Translator.getTranslation(AIRBLOW_NOT_CORRECT), Type.WARNING);
			return false;
		}
		if (!isAirblowConfigured() && putStep.getRobotSettings().isRobotAirblow()) {
			getView().showNotification(Translator.getTranslation(AIRBLOW_NOT_CORRECT), Type.WARNING);
			return false;
		}
		if (putStep.getRobotSettings().isRobotAirblow() && !isInsideMachineBoundaries()) {
	...
	private boolean isAirblowConfigured() {
		for (Clamping clamping : putStep.getRobotSettings().getWorkArea().getAllActiveClampings()) {
			if (!putStep.getRobotSettings().getRobotAirblowSettings().containsKey(clamping.getId())) {
				return false;
			}
		}
	...
	public void changedTIM(final boolean newValue) {
		if (putStep.getRobotSettings().getTurnInMachine() != newValue) {
			putStep.getProcessFlow().processProcessFlowEvent(new DataChangedEvent(putStep.getProcessFlow(), putStep, false));
	...
	private boolean isInsideMachineBoundaries() {
		WorkAreaManager workarea = putStep.getRobotSettings().getWorkArea().getWorkAreaManager();
		if (workarea.getBoundaries() != null) {
			...
			for (int clampingId: putStep.getRobotSettings().getRobotAirblowSettings().keySet()) {
				Clamping clamping = putStep.getRobotSettings().getWorkArea().getWorkAreaManager().getClampingById(clampingId);
	...
	public void changedMachineAirblow(final boolean newValue) {
		if (putStep.getDeviceSettings().getMachineAirblow() != newValue) {
			putStep.getProcessFlow().processProcessFlowEvent(new DataChangedEvent(putStep.getProcessFlow(), putStep, false));
