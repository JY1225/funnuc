...
public class DeviceManager {
	...
	public void updateBasicStackPlate(final BasicStackPlate basicStackPlate, final String name, final String userFrameName, final int horizontalHoleAmount, 
			final int verticalHoleAmount, final float holeDiameter, final float studDiameter, final float horizontalHoleDistance, final float verticalHoleDistance, final float horizontalPadding, 
		...
		try {
			deviceMapper.updateBasicStackPlate(basicStackPlate, name, userFrameName, horizontalHoleAmount, verticalHoleAmount, holeDiameter, studDiameter, 
					horizontalHoleDistance, verticalHoleDistance, horizontalPadding, verticalPaddingTop, verticalPaddingBottom, interferenceDistance, overflowPercentage, horizontalR, tiltedR,
	...
	public void updateCNCMachineData(final AbstractCNCMachine cncMachine, final String name, final EWayOfOperating wayOfOperating,
			...
			final List<Set<Integer>> mCodeRobotServiceOutputs, final boolean workNumberSearch, final boolean clampingPressureSelectable) {
		try {
			deviceMapper.updateCNCMachine(cncMachine, name, wayOfOperating, ipAddress, port, clampingWidthR, 
					...
					robotServiceOutputNames, mCodeNames, mCodeRobotServiceInputs, mCodeRobotServiceOutputs, workNumberSearch, clampingPressureSelectable);
			refresh();
		} catch (SQLException e) {
			logger.error(e);
			e.printStackTrace();
